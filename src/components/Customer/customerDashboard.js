...
import { CustomerAnalyticsGraph, CustomerAnalyticsPie, CustomerDashboardAction, fetchCustomers, setCustomerPagination } from "../../store/actionCreators/Customers/CustomerAction";
...
const CustomerDashboard = (props) => {
    ...
    const customer_analytics_pie = useSelector(state => state.customer_analytics_pie);
    const customer_analytics_graph = useSelector(state => state.customer_analytics_graph);
    ...
    const reports = [
        { icon: FaUsers, title: "Total Customers", value: customerDashboardCards.Total_customers, rate: "2.7%", desc: "From previous period" },
        { icon: MdFastfood, title: "Total Orders", value: customerDashboardCards.Total_orders, rate: "2.4%", desc: "From previous period" },
    ...
    useEffect(() => {
        ...
        dispatch(fetchCustomers(page, currentFilter.franchise.franchise_id, currentFilter.branch.branch_id, "mumbai"))
        dispatch(CustomerDashboardAction(currentFilter.franchise.franchise_id, currentFilter.branch.branch_id, current.month, current.year))
        ...
        if(customers.data){
            return (
                <Container fluid className={props.sideToggle === true ? "closeDash" : "openDash"} style={{ paddingTop: "95px", backgroundColor: "#F1F5F7" }} >
                    <Row>
                        <Col lg={8} sm={6} xs={12} className="dash-head">CUSTOMER</Col>
                        <Col lg={4} sm={6} xs={12}>
                            <Row>
                                <Col>
                    ...
                    <Row >
                        <Col lg="8">
                            <Row>
                                <MiniWidgetsC customer_dashboard={customer_dashboard} reports={reports} />
                            </Row>
         
                            <CustomerAnalytics setRevFilter={setRevFilter} handleYearChange={handleYearChangeRev} handleMonthChange={handleMonthChangeRev} yearDrop={yearDrop} monthDrop={monthDrop} revenueGraph={revenueGraph} revenueAnalytics={revenueAnalytics} />
                        </Col>
                        <Col lg="4">
                            <Row>
                                <Col xl={12}>
                                ...
                                <Col xl={12}>
                                    <Card style={{ width: '100%', boxShadow: " 0 2px 4px rgb(0 0 0 / 8%)" }}>
                                        <Card.Body>
                                            <CustomerDoughnut options={Doughnutstate.options} series={Doughnutstate.series}></CustomerDoughnut>
                                        </Card.Body>
                                    </Card>
                                </Col>
                            </Row>
                    ...
                    <Row className="mt-4">
                    {customers.data.status==="success"?<CustomerTable products={props.products} currentCustomer={props.currentCustomer} setCustomer={props.setCustomer} />:<></>}
