...
export const getEmployeeToken=(email,password,role)=>{
    return (dispatch,getState)=>{
        axios.post(`${process.env.REACT_APP_API_URL}api/v1/admin/admin_login`,{
            "email":email,
            "password":password
        })
        .then(employeeInfo=>{    
            if(employeeInfo.data.status==="success"){
                ...
                .then(empInfo=>{
                    ...
                    axios.get(`${process.env.REACT_APP_API_URL}api/v1/admin/filters/get_filters?role=${empInfo.data.data.admin_role.admin_role}&franchise_id=${empInfo.data.data.franchise_id}&branch=${empInfo.data.data.branch_id}`)
