import { Container, Row, Col, Card, Button, DropdownButton } from "react-bootstrap";
...
const Dashboard = (props) => {
    ...
    const reports = [
        { icon: RiStackLine, title: "Number of Sales", value: salesCarddata.num_of_sales, rate: "2.4%", desc: "From previous period" },
    ...
    useEffect(() => {
        ...
        if (salespie.data) {
            for (let i = 0; i < salespie.data.sales_analytics.length; i++) {
                ...
                labels.push(key[0])
                series.push(parseInt((salespie.data.sales_analytics[i][key[0]]).slice(0, -1)))
    ...
    return (
        <React.Fragment>
            ...
            <Container fluid className={props.sideToggle === true ? "closeDash" : "openDash"} style={{ paddingTop: "95px", backgroundColor: "#F1F5F7" }} >
                <Row>
                    <Col lg={8} sm={6} xs={12} className="dash-head">DASHBOARD</Col>
                    <Col lg={4} sm={6} xs={12}>
                        <Row>
                            <Col>
